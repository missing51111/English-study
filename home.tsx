...
const THEMES = [
  ...
  },
  {
    ...
    bg: "bg-gray-100", card: "bg-white", border: "border-gray-400", innerCard: "bg-gray-200",
    titleText: "text-gray-900", bodyText: "text-gray-900", subText: "text-gray-600",
    accent: "text-gray-700", bar: "bg-gray-600",
    staminaFill: "bg-gray-600", staminaBorder: "border-gray-500",
    reviewFill: "bg-gray-500", reviewBorder: "border-gray-400",
    divider: "border-gray-300",
    nav: "bg-white", navBorder: "border-gray-300", navActive: "text-gray-900", navInactive: "text-gray-400",
    avatarBg: "bg-gray-700", avatarText: "text-white",
...
// 共通部品
// ============================================================
const MissionRow = ({ icon, label, current, target, t }) => (
...
// ParentPanel
// ============================================================
function ParentPanel({ lu, t }) {
  const [open, setOpen] = useState(false);
  const d = MOCK_PARENT;
  const isBaby = lu.id === "baby";
  const isKid = lu.id === "baby" || lu.id === "elementary";
  return (
...
// ============================================================
const ThemePicker = ({ current, onChange }) => (
...
export default function HomePage() {
  ...
  const lu = LEVEL_UI[selectedLevel] || LEVEL_UI.high;
        ...
        <div className="relative">
          ...
          {themeOpen && (
            <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-2xl shadow-xl z-50 p-3 min-w-max">
              ...
              <ThemePicker current={themeId} onChange={id => { setThemeId(id); setThemeOpen(false); }} />
            </div>
          )}
        </div>
...
<circle cx="150" cy="4" r="10" fill="white"/>
      ...
      {/* 保護者パネル（無料のみ） */}
      {isFree && <ParentPanel lu={lu} t={t} />}

      {/* スタートボタン */}
      <div className="flex flex-col gap-2">
        <button className={`w-full ${t.startBtn} ${t.startText} font-bold shadow-lg active:scale-95 transition-all ${lu.startRound} ${lu.startSize}`}>
          {lu.startLabel}
        </button>
      ...
      <nav className={`fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md border-t flex justify-around py-3 px-4 ${t.nav} ${t.navBorder}`}>
        {[
          { icon: "🏠", label: isKid ? "ホーム" : "ホーム",    active: true },
          { icon: "📚", label: isKid ? "たんご" : "単語",      active: false },
          { icon: "🎰", label: isKid ? "がちゃ" : "ガチャ",    active: false },
          { icon: "👤", label: isKid ? "じぶん" : "マイページ", active: false },
        ].map(item => (
          <button key={item.label} className={`flex flex-col items-center gap-1 ${item.active ? t.navActive : t.navInactive}`}>
