...
export const STUDY_IMAGE_STYLE_PROMPT = [
  ...
  "Use a consistent friendly style with soft rounded shapes and clean vector-like digital painting.",
  "Keep the composition simple with one main subject or one easy-to-read scene.",
  "Use bright but gentle colors, soft shadows, and a clean background.",
...
function decideQuestion(record: StudyImageRecord): StudyImageDecision {
  ...
  return {
    shouldGenerate: true,
    skipReason: null,
    subjectPrompt: `Illustrate the main scene of the sentence "${plainSentence}". Keep it literal and easy to understand.`,
  };
}
...
export function buildStudyImagePrompt(record: StudyImageRecord): string {
  ...
  const typeLine =
    record.type === "question"
      ? "This image is for an English sentence question card."
      : record.type === "phrase"
